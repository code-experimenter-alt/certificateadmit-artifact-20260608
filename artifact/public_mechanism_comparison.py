#!/usr/bin/env python3
"""Full public-workload mechanism comparison on UCI Bank Marketing.

The experiment uses job segments as market products and compares report-admission
mechanisms under hidden LDP-budget over-claims. It is a transparent public
workload check, not a claim about a particular commercial platform.
"""
...
OUT_CSV = Path(__file__).with_name("public_mechanism_comparison.csv")
OUT_JSON = Path(__file__).with_name("public_mechanism_comparison.json")
...
def load_rows() -> list[dict[str, str]]:
    if not DATA.exists():
        raise SystemExit(f"missing {DATA}; include UCI Bank Marketing bank-full.csv")
...
def split_bits(bits: list[int], accepted_fraud: float, truthful_switch: float, rejected_fraud: float, rng: random.Random):
    honest_bits = []
    fraud_bits = []
    switched_bits = []
    for bit in bits:
...
def evaluate(rows: list[dict[str, str]], mechanism: str, seed: int) -> dict[str, float]:
    ...
    true_rates = {job: true_conversions[job] / max(1, true_counts[job]) for job in jobs}
    est_rates = {}
    rel_errors = []
    accepted_total = 0
    ...
    for job in jobs:
        ...
        rel_errors.append(abs(est_audience - true_counts[job]) / max(1, true_counts[job]))
        est_rates[job] = est_conversion / max(1.0, est_audience)

    true_top = sorted(jobs, key=lambda j: true_rates[j], reverse=True)[:TOP_K]
    est_top = sorted(jobs, key=lambda j: est_rates[j], reverse=True)[:TOP_K]
    true_best = sum(true_rates[j] for j in true_top) / TOP_K
    selected = sum(true_rates[j] for j in est_top) / TOP_K
    regret = max(0.0, true_best - selected)
    utility = max(0.0, 20.0 - 200.0 * regret - 10.0 * statistics.mean(rel_errors))
    return {
        "accepted_fraud_rate": accepted_fraud_total / max(1, accepted_total),
        "accepted_volume_fraction": accepted_total / (len(rows) * len(jobs)),
