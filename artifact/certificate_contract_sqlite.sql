...
CREATE TABLE CertLedger (rid TEXT NOT NULL, nonce TEXT NOT NULL, src TEXT NOT NULL UNIQUE, seller TEXT NOT NULL, session TEXT NOT NULL, ctr INTEGER NOT NULL, policy TEXT NOT NULL, price_id TEXT NOT NULL, commitment TEXT NOT NULL, status TEXT NOT NULL, PRIMARY KEY(rid, nonce), UNIQUE(seller, session, ctr), UNIQUE(seller, session, nonce));
...
-- CertificateAdmit admission skeleton for sqlite.
BEGIN IMMEDIATE;
-- 0. Durably record receive before parsing, verification, or rejection.
INSERT OR IGNORE INTO AttemptInbox
 (attempt_id, receive_hash, state, received_at)
 VALUES (:attempt_id, :receive_hash, 'received', :now);
